...
def select_parents(fitness_scores,num_parents,method='rank'):
    if method == 'rank':
...
def create_children(all_parents,population):
    children = []
...
def mutation(children_set,mutation_rate):
...
#         f.write(str(password_option)+', '+str(population_size)+', '+str(num_parents)+', '+str(mutation_rate)+', '+str(selection_method))
    ...
    #             print("Passwod resolved in {} generations and {} seconds! \nDiscovered passcode = {}".format(generations,time_taken,password_resolved))
                break
            parents = select_parents(fitness_scores,num_parents,method=selection_method)
            children = create_children(parents,population)
